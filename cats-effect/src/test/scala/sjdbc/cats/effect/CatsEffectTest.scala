...
class CatsEffectTest extends FunSuite:
  ...
  def truncate(conn: Connection): IO[Int] =
    update(sql"truncate table person")(conn)
  ...
  test("autoCommit") {
    ...
    val program2 = dataSource.readOnly[IO].use((query(sql"select id, name from person") >>> seq(parser)).run)
  ...
  test("transaction (Success)") {
    ...
    val program2 =
      dataSource
        .readOnly[IO]
        .use((query(sql"select id, name from person") >>> seq(parser)).run)
  ...
  test("transaction (Failure)") {
    ...
    val program2 =
      dataSource.readOnly[IO].use((query(sql"select id, name from person where id = ${1}") >>> option(parser)).run)
